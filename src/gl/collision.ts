...
export class VoxelGridCollider {
  ...
  moveAabb2(outAabb: Aabb, inAabb: Aabb, velocity: vec3 | Array<number>): void {
    const collisionResult = this.collisionResult;
    const slideAabb = this.slideAabb;
    const slideVel = this.slideVelocity;

    collisionResult.reset();
    this.collidesWithAabb3(collisionResult, inAabb, velocity[0], velocity[1], velocity[2]);

    const t = collisionResult.instant;
    const remainingT = 1 - t;

    const slideAmt = vec3.dot(collisionResult.normal, velocity) * remainingT;

    for (let i = 0; i < 3; i++) {
      slideVel[i] = collisionResult.normal[i] * slideAmt;
    }

    let finalX = velocity[0] * t + inAabb.minX;
    let finalY = velocity[1] * t + inAabb.minY;
    let finalZ = velocity[2] * t + inAabb.minZ;

    const width = inAabb.width();
    const height = inAabb.height();
    const depth = inAabb.depth();

    slideAabb.minX = finalX;
    slideAabb.minY = finalY;
    slideAabb.minZ = finalZ;
    //
    slideAabb.maxX = finalX + width;
    slideAabb.maxY = finalY + height;
    slideAabb.maxZ = finalZ + depth;

    this.collidesWithAabb3(collisionResult, slideAabb, slideVel[0], slideVel[1], slideVel[2]);
    const slideT = collisionResult.instant;

    finalX += slideVel[0] * slideT;
    finalY += slideVel[1] * slideT;
    finalZ += slideVel[2] * slideT;
    
    outAabb.minX = finalX;
    outAabb.minY = finalY;
    outAabb.minZ = finalZ;

    outAabb.maxX = finalX + width;
    outAabb.maxY = finalY + height;
    outAabb.maxZ = finalZ + depth;
  }
}