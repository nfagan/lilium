...
import * as grassSources from './shaders/debug-terrain-grass2';
...
class TerrainGrassDrawable {
  ...
  render(view: mat4, proj: mat4, camera: wgl.ICamera, sunPosition: wgl.types.Real3, sunColor: wgl.types.Real3): void {
    ...
    this.program.set1f('camera_fov', camera.getFieldOfView());
    this.program.setVec3('camera_front', front);

    this.program.set1f('grid_offset', this.gridOffsetZ);
    this.program.set1f('grid_dimension', this.gridScaleZ);
